...
interface VideoPlayerProps {
  ...
  currentTime: number; // in seconds (0 to 90)
...
export const VideoPlayer: React.FC<VideoPlayerProps> = ({
  ...
  // Video playback loop simulation (duration: 90 seconds, from 10:41:00 to 10:42:30)
  ...
  }, [isPlaying, currentTime, playbackSpeed, onSeek]);
  ...
  return (
    <div id="cctv-video-player-container" className="bg-white/80 backdrop-blur-xl border border-slate-200 rounded-2xl overflow-hidden shadow-xs">
      ...
      <div className="px-4 py-2.5 bg-slate-50/80 border-b border-slate-200 flex items-center justify-between">
        <div className="flex items-center gap-2.5">
          ...
          <div className="flex items-center gap-2">
            <span className="text-xs font-semibold text-slate-900">Incident Evidence Clip</span>
            <span className="text-[11px] font-mono font-semibold text-cyan-800 bg-cyan-100 px-2.5 py-0.5 rounded-lg border border-cyan-200">
              {selectedCam}
            </span>
          </div>
        </div>
      ...
      <div className="relative aspect-[16/9] w-full bg-slate-950 flex items-center justify-center">
        ...
        <div className="absolute top-3 right-3 bg-slate-950/80 backdrop-blur-md border border-white/20 rounded-xl px-2.5 py-1 text-[11px] font-mono text-slate-200 flex items-center gap-2 shadow-sm">
          <Eye className="w-3.5 h-3.5 text-cyan-400" />
          <span>Entity Tracking: Active</span>
        </div>
      </div>
      ...
      <div className="px-4 pt-3 pb-2.5 bg-slate-50/80 border-t border-slate-200">
        <div className="relative flex items-center group">
          ...
          {timelineEvents.map((event) => {
            const pct = Math.min(100, Math.max(0, (event.timeSeconds / totalDuration) * 100));
            const isNear = Math.abs(currentTime - event.timeSeconds) < 4;
            return (
              <div
                key={event.id}
        ...
        <div className="flex items-center justify-between mt-2.5">
          <div className="flex items-center gap-3">
            ...
            <div className="font-mono text-xs text-slate-700">
              ...
              <span className="text-slate-500">10:42:30</span>
            </div>
          </div>
